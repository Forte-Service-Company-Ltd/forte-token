...
contract ApplicationERC20MintBurnInvariantTest is ERC20UCommon {
    ...
    function invariant_ERC20external_burn() public {
        uint256 balance_sender = ProtocolTokenv2(address(protocolTokenProxy)).balanceOf(USER1);
        ...
        vm.startPrank(USER1);
        ProtocolTokenv2(address(protocolTokenProxy)).burn(burn_amount);
        assertEq(
            ProtocolTokenv2(address(protocolTokenProxy)).balanceOf(USER1),
            balance_sender - burn_amount
        );
        assertEq(
            ProtocolTokenv2(address(protocolTokenProxy)).totalSupply(),
            supply - burn_amount
        );
    }
    ...
    function invariant_ERC20external_burnFrom() public {
        uint256 balance_sender = ProtocolTokenv2(address(protocolTokenProxy)).balanceOf(USER1);
        uint256 allowance = ProtocolTokenv2(address(protocolTokenProxy)).allowance(USER1, USER2);
        if(!(balance_sender > 0 && allowance > balance_sender))return;
        uint256 supply = ProtocolTokenv2(address(protocolTokenProxy)).totalSupply();
        uint256 burn_amount = amount % (balance_sender + 1);

        ProtocolTokenv2(address(protocolTokenProxy)).burnFrom(USER1, burn_amount);
        assertEq(
            ProtocolTokenv2(address(protocolTokenProxy)).balanceOf(USER1),
            balance_sender - burn_amount
        );
    ...
    function invariant_ERC20external_burnFromUpdateAllowance() public {
        uint256 balance_sender = ProtocolTokenv2(address(protocolTokenProxy)).balanceOf(USER1);
        uint256 current_allowance = ProtocolTokenv2(address(protocolTokenProxy)).allowance(USER1, address(this));
        ...
        if (current_allowance != type(uint256).max) {
            assertEq(
                ProtocolTokenv2(address(protocolTokenProxy)).allowance(USER1, address(this)),
